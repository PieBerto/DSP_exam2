      ...
      <tbody>
        {
          props.reviews.map((review) =>
            <FilmReviewRow reviewData={review} filmData={props.film} key={review.reviewerId} id={review.reviewerId}
              ...
              index={props.reviewSub.findIndex((r)=>review.filmId === r.filmId && review.reviewerId === r.reviewerId)} />
          )
        }
...
function FilmReviewRow(props) {
  ...
  useEffect(()=>{
    if(props.index !== -1){
      const review = props.reviewSub.find((review)=>review.filmId === props.reviewData.filmId && review.reviewerId === props.reviewData.reviewerId);
      if(review){
        setSub(review.subscribed);
      }
    }
  },[props.index,props.reviewSub[props.index]])
  ...
  return (
    <tr>
      ...
      <td>
        <Form.Switch type={'switch'} label={'Notify'} disabled={props.switchDisabled} checked={sub} onChange={()=>props.reviewReverseSub(props.reviewData.filmId, props.reviewData.reviewerId)} />
      </td>
    </tr>
  );
}
