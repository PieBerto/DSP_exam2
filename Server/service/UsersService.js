 ...
 */
   exports.getActiveFilmUser = function(userId) {
    ...
    /**
 * Retrieve the films for which the user is a reviewer
 * 
 * Input:
 * - userId of the user
 * Output:
 * - the films for which the user is a reviewer
 * 
 */
  exports.getReviewerFilm = (userId) => {
    return new Promise((resolve,reject)=>{
        const sql = "SELECT filmId FROM reviews WHERE reviewerId = ?";
        db.all(sql,[userId], (err,rows) => {
            if(err){
                reject(err);
            } else if(rows.length === 0){
